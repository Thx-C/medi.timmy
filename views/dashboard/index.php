...
// Fonction d'échappement si non définie globalement
...
$dateAujourdhui = strftime('%A %d %B %Y');

$pageTitle   = 'Tableau de bord';
$activeRoute = 'dashboard';

// Tableaux de badges sortis de la boucle
$badgeClasses = [
    'planifie' => 'badge-blue',
...
<div class="page-header">
  <h1>Bonjour, <?= e($_SESSION['user']['prenom'] ?? 'Utilisateur') ?> 👋</h1>
  <p>Voici un aperçu de l'activité du cabinet — <?= e($dateAujourdhui) ?></p>
</div>
...
<div class="stats-grid">
  <div class="stat-card">
    ...
    <div>
      <div class="stat-value"><?= (int)$stats['aujourdhui'] ?></div>
  ...
  <div class="stat-card">
    ...
    <div>
      <div class="stat-value"><?= (int)$stats['semaine'] ?></div>
  ...
  <div class="stat-card">
    ...
    <div>
      <div class="stat-value"><?= (int)$stats['confirmes'] ?></div>
  ...
  <div class="stat-card">
    ...
    <div>
      <div class="stat-value"><?= (int)$stats['planifies'] ?></div>
  ...
  <div class="stat-card">
    ...
    <div>
      <div class="stat-value"><?= (int)$stats['patients'] ?></div>
  ...
  <div class="stat-card">
    ...
    <div>
      <div class="stat-value"><?= (int)$stats['annules'] ?></div>
    ...
    <div class="card-body">
      ...
      <?php else: ?>
        <div class="table-wrap">
          <table>
            <thead>
              <tr><th>Date</th><th>Patient</th><th>Praticien</th><th>Statut</th></tr>
            </thead>
            <tbody>
            <?php foreach ($prochains as $rdv): ?>
              <?php
                // On calcule le timestamp une seule fois et on vérifie sa validité
                $ts     = !empty($rdv['date_heure']) ? strtotime($rdv['date_heure']) : false;
                $statut = $rdv['statut'] ?? 'inconnu';
              ?>
              <tr>
                <td>
                  <?php if ($ts !== false): ?>
                    <div class="font-bold text-sm"><?= date('d/m', $ts) ?></div>
                    <div class="text-xs text-gray"><?= date('H:i', $ts) ?></div>
                  <?php else: ?>
                    <div class="text-xs text-gray">Date invalide</div>
                  <?php endif; ?>
                </td>
                <td class="font-bold"><?= e($rdv['patient_nom'] ?? '') ?></td>
                <td class="text-sm text-gray"><?= e($rdv['praticien_nom'] ?? '') ?></td>
                <td>
                  <span class="badge <?= e($badgeClasses[$statut] ?? 'badge-gray') ?>">
                    <?= e($badgeLabels[$statut] ?? ucfirst($statut)) ?>
                  </span>
                </td>
              </tr>
            <?php endforeach; ?>
    ...
    <div class="card-body" style="display:flex; flex-direction:column; gap:10px;">
      <?php if (RoleMiddleware::can('gerer_agenda')): ?>
        <button class="btn btn-primary"
                onclick="document.getElementById('modal-rdv').classList.add('open')">
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
